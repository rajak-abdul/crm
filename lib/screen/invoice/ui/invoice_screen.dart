...
import 'package:crm_app/shareWidgets/share_widgets.dart' show showApiSnack, LoadingState, ErrorState;
...
class InvoiceLoaded extends InvoiceState {
  ...
  @override List<Object?> get props =>
      [invoices, recentInvoices, pendingInvoices, salesUsers, deals];
}
...
class _InvoiceView extends StatefulWidget {
  const _InvoiceView();
  @override State<_InvoiceView> createState() => _InvoiceViewState();
}
...
class _InvoiceViewState extends State<_InvoiceView> {
  ...
  List<Invoice> _filtered(List<Invoice> all) {
    ...
    return all.where((inv) {
      ...
      final matchStatus = _statusFilter == 'All Status' || inv.status == _statusFilter;
      bool matchDate = true;
    ...
    builder: (_) => BlocProvider.value(
      ...
      child: _InvoiceFormModal(
        ...
        onSave: (data) async {
          ...
          if (mounted) showApiSnack(context, 'Invoice created successfully');
        },
      ),
    ),
  );
  ...
  void _openEdit(Invoice inv, InvoiceLoaded s) => showModalBottomSheet(
    ...
    builder: (_) => BlocProvider.value(
      ...
      child: _InvoiceFormModal(
        ...
        onSave: (data) async {
          Navigator.pop(context);
          await context.read<InvoiceCubit>().updateInvoice(inv.id, data);
          if (mounted) showApiSnack(context, 'Invoice updated');
        },
      ),
    ),
  );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<InvoiceCubit, InvoiceState>(
        builder: (ctx, state) {
          if (state is InvoiceLoading) return const LoadingState(message: 'Loading invoices…');
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: (_selectedDate != null) ? AppColors.primaryLight : AppColors.background,
                      ...
                      child: SizedBox(
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            prefixIcon: const Icon(Icons.search, size: 17, color: AppColors.textSecondary),
      ...
      style: ElevatedButton.styleFrom(
        ...
        minimumSize: Size.zero,
              ...
              Padding(
                ...
                child: Row(children: [
                  Text('${filtered.length} invoice${filtered.length != 1 ? 's' : ''}',
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                          color: AppColors.textSecondary)),
                  const Spacer(),
                  if (_q.isNotEmpty || _statusFilter != 'All Status' || _selectedDate != null)
                    GestureDetector(
                      onTap: () => setState(() {
              ...
              Expanded(child: filtered.isEmpty
                ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.receipt_long_outlined, size: 52, color: AppColors.textHint),
                    SizedBox(height: 10),
  ...
  Future<void> _downloadPdf(BuildContext ctx, Invoice inv) async {
    if (inv.isLocal) {
      showApiSnack(context, 'PDF only available after invoice is synced to server.', isError: true);
      return;
    }
  ...
  Future<void> _sendEmail(BuildContext ctx, Invoice inv) async {
    if (inv.isLocal) {
      showApiSnack(context, 'Sync invoice before sending email.', isError: true);
      return;
    }
...
class _SummaryCarousel extends StatefulWidget {
  ...
  @override State<_SummaryCarousel> createState() => _SummaryCarouselState();
}
...
class _SummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colors =
        (slide['colors'] as List<Color>?) ?? [const Color(0xFF1D4ED8), const Color(0xFF0EA5E9)];
  ...
  Widget _gradientCard({required List<Color> colors, required Widget child}) =>
    Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: colors.first.withOpacity(0.3),
...
class _InvoiceCard extends StatefulWidget {
  ...
  @override State<_InvoiceCard> createState() => _InvoiceCardState();
}
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(inv.dealName.isNotEmpty ? inv.dealName : 'No Deal',
                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700,
                      color: AppColors.textPrimary), maxLines: 1, overflow: TextOverflow.ellipsis),
              Text(inv.assignTo.isNotEmpty ? inv.assignTo : 'Unassigned',
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              _StatusBadge(status: inv.status, color: _statusColor, icon: _statusIcon),
            ]),
          ]),
        ),
        Padding(
          ...
          child: Row(children: [
            ...
            Flexible(child: _MetaChip(
                ...
                valueColor: inv.status != 'Paid' ? AppColors.danger : AppColors.textSecondary)),
            const Spacer(),
            GestureDetector(
            ...
            ? Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: TextButton.icon(
                    ...
                    label: const Text(' Send mail', style: TextStyle(fontSize: 13)),
                    style: TextButton.styleFrom(
...
class _StatusBadge extends StatelessWidget {
  final String status; final Color color; final IconData icon;
  const _StatusBadge({required this.status, required this.color, required this.icon});
  @override
...
class _MetaChip extends StatelessWidget {
  ...
  const _MetaChip({required this.icon, required this.label, required this.value, this.valueColor});
  @override
...
class _InvoiceFormModal extends StatefulWidget {
  ...
  const _InvoiceFormModal({
    this.existing, required this.salesUsers,
    required this.deals, required this.onSave,
  });
  @override State<_InvoiceFormModal> createState() => _InvoiceFormModalState();
}
...
class _InvoiceFormModalState extends State<_InvoiceFormModal> {
  ...
  static const _currencies    = ['INR', 'USD', 'EUR', 'GBP', 'JPY', 'CNY'];
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      _disValCtrl.text  = e.discountValue == 0 ? '' : e.discountValue.toStringAsFixed(2);
  ...
  @override
  void dispose() {
    _priceCtrl.dispose(); _taxValCtrl.dispose();
    _disValCtrl.dispose(); _notesCtrl.dispose();
    super.dispose();
  }
  ...
  double get _taxAmount {
    if (_taxType == 'Percentage')   return _price * _taxVal / 100;
  ...
  double get _total => (_price + _taxAmount - _discountAmount).clamp(0, double.infinity);
  ...
  void _save() {
    if (_selectedUser == null)      { _snack('Assign To is required');  return; }
    if (_issueDate.isEmpty)         { _snack('Issue Date is required'); return; }
    if (_dueDate.isEmpty)           { _snack('Due Date is required');   return; }
    if (_selectedDeal == null)      { _snack('Select a Deal');          return; }
    if (_priceCtrl.text.isEmpty)    { _snack('Price is required');      return; }
  ...
  void _snack(String msg) => showApiSnack(context, msg, isError: true);
  ...
  Future<void> _pickDate(bool isIssue) async {
    ...
    if (picked != null) {
      ...
      setState(() { if (isIssue) { _issueDate = s; } else { _dueDate = s; } });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final userNames = ['Select User', ...widget.salesUsers.map((u) => u.name)];
    final displayName = _selectedUser?.name ?? 'Select User';

    return DraggableScrollableSheet(
      initialChildSize: 0.97, maxChildSize: 0.99, minChildSize: 0.5,
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(_isEdit ? 'Edit Invoice' : 'Create New Invoice',
                    style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                Text(_isEdit ? 'Update invoice details' : 'Fill in invoice details below',
          ...
          Expanded(child: ListView(controller: ctrl, padding: const EdgeInsets.all(16), children: [
            ...
            const _SecHeader(title: 'Basic Information', icon: Icons.info_outline),
            const SizedBox(height: 14),

            const _FieldLabel('Assign To (Sales User)', required: true),
            const SizedBox(height: 6),
            // ── FIX: dropdown now maps selected name → SalesUser ──
            _FullDrop(
              value:    userNames.contains(displayName) ? displayName : 'Select User',
              items:    userNames,
              ...
              onChanged: (v) {
                if (v == 'Select User') {
                  ...
                  try {
                    final user = widget.salesUsers.firstWhere((u) => u.name == v);
                    setState(() => _selectedUser = user);
                  } catch (_) {
                    setState(() => _selectedUser = SalesUser('', v));
                  }
                }
              },
            ),
            ...
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 6),
                _DateTile(value: _issueDate.isEmpty ? 'Pick date' : _issueDate, onTap: () => _pickDate(true)),
              ])),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const _FieldLabel('Due Date', required: true),
                const SizedBox(height: 6),
                _DateTile(value: _dueDate.isEmpty ? 'Pick date' : _dueDate, onTap: () => _pickDate(false)),
              ])),
            ]),
            const SizedBox(height: 14),
            ...
            const SizedBox(height: 6),
            _FullDrop(value: _status, items: _statuses, icon: Icons.info_outline,
            ...
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 6),
                _FullDrop(value: _taxType, items: _taxTypes, icon: Icons.percent_outlined,
                    onChanged: (v) => setState(() { _taxType = v; _taxValCtrl.clear(); })),
              ])),
              if (_taxType != 'Zero Tax') ...[
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _FieldLabel(_taxType == 'Percentage' ? 'Tax %' : 'Tax Amount'),
                  const SizedBox(height: 6),
            ...
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 6),
                _FullDrop(value: _discountType, items: _discountTypes, icon: Icons.local_offer_outlined,
                    onChanged: (v) => setState(() { _discountType = v; _disValCtrl.clear(); })),
              ])),
              if (_discountType != 'No Discount') ...[
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  _FieldLabel(_discountType == 'Percentage' ? 'Discount %' : 'Discount Amt'),
                  const SizedBox(height: 6),
            ...
            const _SecHeader(title: 'Deal Information', icon: Icons.handshake_outlined),
            const SizedBox(height: 14),
            ...
            Container(
              ...
              child: DropdownButtonHideUnderline(child: DropdownButton<DealOption>(
                value: _selectedDeal,
                isExpanded: true, isDense: true,
                hint: const Row(children: [
                  Icon(Icons.handshake_outlined, size: 14, color: AppColors.textHint),
                  SizedBox(width: 8),
                  Text('Select Deal', style: TextStyle(fontSize: 14, color: AppColors.textHint)),
                ]),
                icon: const Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: AppColors.textSecondary),
                style: const TextStyle(fontSize: 14, color: AppColors.textPrimary),
                items: widget.deals.map((d) => DropdownMenuItem(value: d,
                  child: Row(children: [
                    const Icon(Icons.handshake_outlined, size: 14, color: AppColors.textHint),
                    const SizedBox(width: 8),
            ...
            Row(children: [
              ...
              const SizedBox(width: 10),
              Expanded(child: _tfNum(_priceCtrl, '0.00', onChanged: (_) => setState(() {}))),
            ]),
            ...
            Container(
              ...
              child: Column(children: [
                _TotalRow(label: 'Price', value: _price, currency: _currency),
                if (_taxAmount     != 0) _TotalRow(label: 'Tax',      value: _taxAmount,      currency: _currency, isAdd: true),
                if (_discountAmount != 0) _TotalRow(label: 'Discount', value: _discountAmount, currency: _currency, isAdd: false),
                const Divider(height: 16, color: AppColors.divider),
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Text('Total', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.textPrimary)),
                  Text('${_symFor(_currency)}${_fmtCur(_total)}',
            ...
            Row(children: [
              ...
              Expanded(child: ElevatedButton.icon(
                ...
                label: Text(_saving ? 'Saving…' : (_isEdit ? 'Update Invoice' : 'Create Invoice')),
              )),
            ]),
  ...
  Widget _tfNum(TextEditingController c, String hint, {void Function(String)? onChanged}) =>
    TextField(
...
class _TotalRow extends StatelessWidget {
  ...
  final double value; final bool isAdd;
  const _TotalRow({required this.label, required this.value, required this.currency, this.isAdd = true});
  @override
    ...
    child: Container(
      ...
      child: Row(children: [
        ...
        Text(value, style: TextStyle(fontSize: 13,
            color: value.contains('Pick') ? AppColors.textHint : AppColors.textPrimary)),
      ]),
    ),
  );
}
...
class _FDrop extends StatelessWidget {
  ...
  final IconData icon; final void Function(String) onChanged;
  const _FDrop({required this.value, required this.items, required this.icon, required this.onChanged});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: DropdownButtonHideUnderline(child: DropdownButton<String>(
      ...
      style: TextStyle(fontSize: 13, fontWeight: _active ? FontWeight.w600 : FontWeight.w400,
          color: _active ? AppColors.primary : AppColors.textSecondary),
...
class _FullDrop extends StatelessWidget {
  final String value; final List<String> items;
  final IconData icon; final void Function(String) onChanged;
  const _FullDrop({required this.value, required this.items, required this.icon, required this.onChanged});
  @override
  Widget build(BuildContext context) => Container(
