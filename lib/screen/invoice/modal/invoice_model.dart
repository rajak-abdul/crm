...
class Invoice {
  final String  id, invoiceNo, assignTo, issueDate,
                dueDate, status, taxType, discountType,
                dealId, dealName, notes, currency;
  ...
  static String normaliseStatus(String s) {
    switch (s.toLowerCase().trim()) {
      ...
      default:       return s.isEmpty ? 'Unpaid' : s;
    }
  }
  ...
  double get taxAmount {
    if (taxType == 'Percentage')   return price * taxValue / 100;
  ...
  double get total => (price + taxAmount - discountAmount).clamp(0, double.infinity);
  ...
  factory Invoice.fromJson(Map<String, dynamic> j) {
    ...
    if (items is List && items.isNotEmpty && items.first is Map) {
      ...
      if (rawDeal is Map) {
        ...
        dealName = _s(rawDeal['dealName'] ?? rawDeal['name'] ?? rawDeal['title']);
    ...
    if (price == 0) {
      for (final k in ['totalAmount', 'subtotal', 'amount', 'price', 'invoiceAmount', 'total']) {
        final v = _d(j[k]);
        if (v != null && v != 0) {
          price = v;
          break;
    ...
    // tax / discount values
    final taxVal  = (j['taxValue']      as num?)?.toDouble() ??
                    (j['tax_value']     as num?)?.toDouble() ??
                    (j['tax']           as num?)?.toDouble() ?? 0;
    final disVal  = (j['discountValue'] as num?)?.toDouble() ??
                    (j['discount_value']as num?)?.toDouble() ??
    ...
    return Invoice(
      ...
      invoiceNo:     invoiceNo,
      assignTo:      assignTo,
      issueDate:     _fmtRawDate(j['issueDate']  ?? j['issue_date']
                                 ?? j['createdAt'] ?? ''),
  ...
  Map<String, dynamic> toSaveMap() => {
    'assignTo':      assignTo,     'issueDate':     issueDate,
    'dueDate':       dueDate,      'status':        status,
    'taxType':       taxType,      'taxValue':      taxValue,
  ...
  Invoice copyWith({
    String? assignTo, String? issueDate, String? dueDate, String? status,
    String? taxType, double? taxValue, String? discountType, double? discountValue,
    String? dealId, String? dealName, double? price, String? notes, String? currency,
    double? inrAmount, double? exchangeRate, bool? isLocal,
  }) => Invoice(
    id: id, invoiceNo: invoiceNo, createdAt: createdAt, isLocal: isLocal ?? this.isLocal,
