...
class _DealsScreenState extends State<DealsScreen> {
  ...
  Widget build(BuildContext context) => BlocProvider.value(
        value: _cubit,
        child: _DealsShell(initialTab: widget.initialTab));
}

class _DealsShell extends StatefulWidget {
  final int initialTab;
  const _DealsShell({required this.initialTab});
  @override State<_DealsShell> createState() => _DealsShellState();
}
...
class _DealsShellState extends State<_DealsShell> {
  ...
  Widget _buildLoaded(BuildContext ctx, DealsLoaded loaded) =>
      AnimatedSwitcher(
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              child: _ViewDropdown(
                value: _view, options: _viewOptions,
        ...
        body: BlocConsumer<DealsCubit, DealsState>(
          ...
          builder: (ctx, state) {
            ...
            if (_lastLoaded != null)
              return _buildLoaded(ctx,
                  state is DealsLoaded ? state : _lastLoaded!);
...
class _ViewDropdown extends StatelessWidget {
  ...
  const _ViewDropdown({required this.value, required this.options, required this.onChanged});
...
class _StagesView extends StatefulWidget {
  ...
  const _StagesView({super.key, required this.deals, required this.pendingDeals,
      required this.lostDeals, required this.onCreateDeal,
      required this.onViewDeal,
      required this.onEditDeal, required this.onDeleteDeal});
  @override State<_StagesView> createState() => _StagesViewState();
}
...
class _StagesViewState extends State<_StagesView> {
  ...
  Widget build(BuildContext context) => Column(children: [
        Container(color: AppColors.surface,
          ...
          child: Row(children: [
            Expanded(child: _SearchBar(controller: _search, hint: 'Search deals…',
              onChanged: (v) => setState(() => _q = v.toLowerCase()),
              onClear: () => setState(() => _q = ''))),
            const SizedBox(width: 10),
            if (PermissionHelper.can('admin_access'))
            ElevatedButton.icon(onPressed: widget.onCreateDeal,
              icon: const Icon(Icons.add, size: 16),
              label: const Text('Create Deal', style: TextStyle(fontSize: 13)),
        ...
        if (widget.pendingDeals.isNotEmpty || widget.lostDeals.isNotEmpty)
          Container(color: AppColors.surface,
            ...
            child: Row(children: [
              _StripChip(label: 'Pending Follow-up',
                  count: widget.pendingDeals.length, color: AppColors.warning),
              const SizedBox(width: 10),
              _StripChip(label: 'Lost Deals',
                  count: widget.lostDeals.length, color: AppColors.danger),
            ])),
        Expanded(child: RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              final filtered = widget.deals.where((d) =>
                DealConstants.canonicalStage(d.stage) == stage &&
                    (_q.isEmpty ||
                  d.name.toLowerCase().contains(_q) ||
                  d.companyName.toLowerCase().contains(_q))).toList();
              return _StageColumn(stage: stage, deals: filtered,
                  onView: widget.onViewDeal,
                  onEdit: widget.onEditDeal, onDelete: widget.onDeleteDeal);
            }))),
      ]);
}
...
class _StripChip extends StatelessWidget {
  ...
  const _StripChip({required this.label, required this.count, required this.color});
  @override
  Widget build(BuildContext context) => Container(
...
class _StageColumn extends StatefulWidget {
  final String stage; final List<Deal> deals;
  final void Function(Deal) onView;
  final void Function(Deal) onEdit; final void Function(String) onDelete;
  const _StageColumn({required this.stage, required this.deals,
      required this.onView,
      required this.onEdit, required this.onDelete});
  @override State<_StageColumn> createState() => _StageColumnState();
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        GestureDetector(onTap: () => setState(() => _collapsed = !_collapsed),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 13),
            decoration: BoxDecoration(color: color.withOpacity(0.07),
              borderRadius: _collapsed ? BorderRadius.circular(16)
                  : const BorderRadius.vertical(top: Radius.circular(16))),
            child: Row(children: [
              Container(padding: const EdgeInsets.all(7),
                decoration: BoxDecoration(color: color.withOpacity(0.15),
                    borderRadius: BorderRadius.circular(9)),
                child: Icon(_stageIcon(widget.stage), size: 16, color: color)),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(widget.stage, style: TextStyle(fontSize: 14,
                    fontWeight: FontWeight.w700, color: color)),
                Text('${widget.deals.length} deal${widget.deals.length != 1 ? 's' : ''}',
              ...
              AnimatedRotation(turns: _collapsed ? 0 : 0.5,
                  duration: const Duration(milliseconds: 200),
        ...
        AnimatedSize(
          ...
          child: _collapsed ? const SizedBox.shrink() : Column(
            children: widget.deals.isEmpty
                ? [const Padding(padding: EdgeInsets.all(20),
                    child: Center(child: Text('No deals in this stage',
                        style: TextStyle(color: AppColors.textHint, fontSize: 13))))]
                : widget.deals.map((d) => _DealTile(deal: d,
                    onView: widget.onView,
                    onEdit: widget.onEdit, onDelete: widget.onDelete)).toList())),
      ]));
  }
}
...
class _DealTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Material(color: Colors.transparent, borderRadius: BorderRadius.circular(12),
        child: InkWell(borderRadius: BorderRadius.circular(12), onTap: () => onView(deal),
          child: Padding(padding: const EdgeInsets.all(13),
            child: Row(children: [
              Container(width: 40, height: 40,
                decoration: BoxDecoration(color: color.withOpacity(0.12),
                    borderRadius: BorderRadius.circular(10)),
                child: Center(child: Text(deal.name.isNotEmpty ? deal.name[0].toUpperCase() : '?',
                    style: TextStyle(color: color, fontWeight: FontWeight.w800, fontSize: 17)))),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(deal.name, style: const TextStyle(fontSize: 13,
                    fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                Text(deal.companyName, style: const TextStyle(fontSize: 11,
                    color: AppColors.textSecondary)),
                const SizedBox(height: 4),
                Row(children: [
                  const Icon(Icons.person_outline, size: 11, color: AppColors.textHint),
                  const SizedBox(width: 3),
                  Flexible(child: Text(deal.assignTo.isNotEmpty ? deal.assignTo : 'Unassigned',
                      overflow: TextOverflow.ellipsis,
                      style: const TextStyle(fontSize: 11, color: AppColors.textHint))),
                ]),
              ])),
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text(_fmtVal(deal.currency, deal.value), overflow: TextOverflow.ellipsis,
                    style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: color)),
                const SizedBox(height: 6),
                Row(mainAxisSize: MainAxisSize.min, children: [
                  GestureDetector(onTap: () => onEdit(deal), behavior: HitTestBehavior.opaque,
                      child: const Icon(Icons.edit_outlined, size: 16, color: AppColors.textHint)),
                  const SizedBox(width: 10),
                  GestureDetector(onTap: () => _confirmDelete(context), behavior: HitTestBehavior.opaque,
                      child: const Icon(Icons.delete_outline, size: 17, color: AppColors.textHint)),
                ]),
              ]),
            ])))));
  }
  ...
  Future<void> _confirmDelete(BuildContext ctx) async {
    final ok = await showDialog<bool>(context: ctx, builder: (_) => AlertDialog(
      ...
      title: const Text('Delete Deal'), content: Text('Remove "${deal.name}"?'),
      actions: [
...
class _AllDealsView extends StatefulWidget {
  final List<Deal> deals; final List<String> salesUsers;
  final VoidCallback onCreateDeal;
  final void Function(Deal) onViewDeal;
  final void Function(Deal) onEditDeal; final void Function(String) onDeleteDeal;
  const _AllDealsView({super.key, required this.deals, required this.salesUsers,
      required this.onCreateDeal, required this.onViewDeal,
      required this.onEditDeal, required this.onDeleteDeal});
  @override State<_AllDealsView> createState() => _AllDealsViewState();
}
...
class _AllDealsViewState extends State<_AllDealsView> {
  ...
  @override void didUpdateWidget(_AllDealsView old) {
  ...
  List<Deal> get _filtered {
    ...
    final safeStage = allStages.contains(_stageFilter) ? _stageFilter : 'All Stages';
    ...
    return widget.deals.where((d) {
      ...
      final matchTodayFollowUps = !_todayFollowupsOnly ||
          (followUpDay != null && followUpDay == today);
      return matchQ &&
          matchStage &&
          matchUser &&
          matchClientType &&
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Container(color: AppColors.surface, padding: const EdgeInsets.fromLTRB(16, 12, 16, 0),
        child: Row(children: [
          Expanded(child: _SearchBar(controller: _searchCtrl, hint: 'Search deal name…',
      ...
      Container(color: AppColors.surface, padding: const EdgeInsets.fromLTRB(16, 10, 16, 14),
        child: Row(children: [
          Expanded(child: _FDrop(
            value: allStages.contains(_stageFilter) ? _stageFilter : 'All Stages',
            items: allStages, icon: Icons.flag_outlined,
            onChanged: (v) => setState(() {
              _stageFilter = v;
              _page = 1;
            }))),
          const SizedBox(width: 10),
          Expanded(child: _FDrop(
            value: allUsers.contains(_userFilter) ? _userFilter : 'All Users',
            items: allUsers, icon: Icons.person_outline,
      ...
      Container(color: AppColors.surface, padding: const EdgeInsets.fromLTRB(16, 0, 16, 14),
        child: Row(children: [
          Expanded(
              child: InkWell(
            borderRadius: BorderRadius.circular(10),
          ...
          const SizedBox(width: 10),
          Expanded(child: _FDrop(
            value: allClientTypes.contains(_clientTypeFilter) ? _clientTypeFilter : 'All Client Types',
            items: allClientTypes, icon: Icons.corporate_fare_outlined,
            onChanged: (v) => setState(() {
              _clientTypeFilter = v;
              _page = 1;
            }))),
        ...
        child: Row(children: [
          Text('${_filtered.length} deal${_filtered.length != 1 ? 's' : ''}',
              style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                  color: AppColors.textSecondary)),
          const Spacer(),
          if (_hasFilter) GestureDetector(
            onTap: () => setState(() { _stageFilter = 'All Stages';
              _userFilter = 'All Users'; _q = ''; _searchCtrl.clear();
              _clientTypeFilter = 'All Client Types';
      ...
      Expanded(child: _filtered.isEmpty
          ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.handshake_outlined, size: 52, color: AppColors.textHint),
              SizedBox(height: 10),
              Text('No deals found', style: TextStyle(color: AppColors.textSecondary, fontSize: 15))]))
          : RefreshIndicator(color: AppColors.primary,
              onRefresh: () => context.read<DealsCubit>().refresh(),
              child: ListView.builder(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
                itemCount: _pagedDeals.length,
                itemBuilder: (_, i) => _DealRow(deal: _pagedDeals[i],
                    onView: widget.onViewDeal,
                    onEdit: widget.onEditDeal, onDelete: widget.onDeleteDeal)))),
...
class _DealRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final followUpLabel = followUpDt == null
        ...
        : DateFormat('dd MMM yy').format(
            followUpDt.isUtc ? followUpDt.toLocal() : followUpDt);
    return Container(
      ...
      child: Material(color: Colors.transparent, borderRadius: BorderRadius.circular(14),
        child: InkWell(borderRadius: BorderRadius.circular(14), onTap: () => onView(deal),
          child: Padding(padding: const EdgeInsets.all(14),
            child: Column(children: [
              Row(children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(deal.name, overflow: TextOverflow.ellipsis,
                      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700,
                          color: AppColors.textPrimary)),
                  Text(deal.companyName, overflow: TextOverflow.ellipsis,
              ...
              const SizedBox(height: 10),
              Row(children: [
                Flexible(child: _meta(Icons.person_outline,
                    deal.assignTo.isNotEmpty ? deal.assignTo : 'Unassigned')),
                const SizedBox(width: 8),
  ...
  Widget _meta(IconData ic, String t) => Row(mainAxisSize: MainAxisSize.min, children: [
    Icon(ic, size: 12, color: AppColors.textHint), const SizedBox(width: 4),
...
class _DealDetailSheet extends StatelessWidget {
                  ...
                  Padding(
                    ...
                    child: Row(children: [
                      Container(
                          ...
                          child: Center(
                              child: Text(
                            deal.name.isNotEmpty ? deal.name[0].toUpperCase() : '?',
  ...
  Widget _buildDetailsTab(ScrollController ctrl) => ListView(
        ...
        children: [
          _row(Icons.payments_outlined, 'Deal Value', _fmtVal(deal.currency, deal.value)),
          if (_followUpLocal(deal.followUpDate) != null)
            _row(
              Icons.event_outlined,
              'Follow Up',
              DateFormat('dd MMM yyyy, hh:mm a').format(_followUpLocal(deal.followUpDate)!),
            ),
          _row(
              Icons.person_outline,
              'Assigned To',
              deal.assignTo.trim().isEmpty ? 'Unassigned' : deal.assignTo.trim()),
          _row(Icons.phone_outlined, 'Phone', deal.phone.trim().isEmpty ? '-' : deal.phone),
          _row(Icons.email_outlined, 'Email', deal.email.trim().isEmpty ? '-' : deal.email),
          _row(Icons.public_outlined, 'Country', deal.country.trim().isEmpty ? '-' : deal.country),
          if (deal.industry.isNotEmpty) _row(Icons.business_outlined, 'Industry', deal.industry),
          if (deal.source.isNotEmpty) _row(Icons.radar_outlined, 'Source', deal.source),
          if (deal.address.isNotEmpty) _row(Icons.location_on_outlined, 'Address', deal.address),
          if (deal.notes.isNotEmpty) _row(Icons.notes_outlined, 'Notes', deal.notes),
  ...
  Widget _buildFollowupHistoryTab(ScrollController ctrl) {
    DateTime? pickDt(FollowUpHistoryItem h) =>
        (h.followUpDate ?? h.date);
    ...
    List<Widget> cardsFor(List<FollowUpHistoryItem> items, {required bool upcoming}) {
...
class _DealFormModal extends StatefulWidget {
  ...
  @override State<_DealFormModal> createState() => _DealFormModalState();
}
...
class _DealFormModalState extends State<_DealFormModal> {
  ...
  @override
  void dispose() {
    for (final c in [_name, _value, _phone, _email,
        _company, _address, _notes, _followUpComment]) c.dispose();
    super.dispose();
  }
  ...
  void _prefill(Deal d) {
    ...
    _currency    = DealConstants.currencies.contains(d.currency)
        ? d.currency : DealConstants.currencies.first;
    _countryCode = DealConstants.countryCodes.contains(d.countryCode)
        ? d.countryCode : DealConstants.countryCodes.first;
    _stage = DealConstants.stages
            .contains(DealConstants.canonicalStage(d.stage))
        ? DealConstants.canonicalStage(d.stage)
        : DealConstants.stages.first;
    _industry    = DealConstants.industries.contains(d.industry)
        ? d.industry : DealConstants.industries.first;
    _source      = AppConstants.leadSources.contains(d.source)
        ? d.source : AppConstants.leadSources.first;
    _clientType  = (d.clientType == 'B2B' || d.clientType == 'B2C')
        ? d.clientType
        : 'B2B';
    _country     = AppConstants.countries.contains(d.country)
        ? d.country : AppConstants.countries.first;

    if (d.assignTo.isNotEmpty) {
      _assignTo = widget.users.where((u) =>
          _userName(u).trim().toLowerCase() == d.assignTo.trim().toLowerCase())
          .firstOrNull;
    }
  ...
  Future<void> _save() async {
    if (_name.text.trim().isEmpty)    { _snack('Deal Name is required');    return; }
    if (_value.text.trim().isEmpty)   { _snack('Deal Value is required');   return; }
    if (_phone.text.trim().isEmpty)   { _snack('Phone Number is required'); return; }
    if (_email.text.trim().isEmpty)   { _snack('Email is required');        return; }
    if (_company.text.trim().isEmpty) { _snack('Company Name is required'); return; }

    setState(() => _saving = true);
    final cubit = context.read<DealsCubit>();

    final selectedUser = _assignTo ?? (widget.users.isNotEmpty ? widget.users.first : null);
    final assignToId   = (selectedUser != null && selectedUser.isNotEmpty)
        ? _userId(selectedUser) : null;
    ...
    final payload = <String, dynamic>{
      ...
      'dealValue':   (double.tryParse(_value.text.replaceAll(',', '')) ?? 0).toString(),
  ...
  Widget build(BuildContext context) => ScaffoldMessenger(
        ...
        initialChildSize: 0.96, maxChildSize: 0.98, minChildSize: 0.5,
        builder: (_, ctrl) => Container(
          decoration: const BoxDecoration(
            color: AppColors.background,
            borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
          child: Padding(
            ...
            child: Column(children: [
              ...
              Container(color: AppColors.surface,
                ...
                child: Row(children: [
                  Container(padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      color: _isEdit ? AppColors.warningLight : AppColors.primaryLight,
                      borderRadius: BorderRadius.circular(10)),
                    child: Icon(_isEdit ? Icons.edit_outlined : Icons.handshake_outlined,
                        color: _isEdit ? AppColors.warning : AppColors.primary, size: 20)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(_isEdit ? 'Edit Deal' : 'Create New Deal',
                        style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800,
                            color: AppColors.textPrimary)),
                    Text(_isEdit ? 'Update deal details below' : 'Fill in deal details below',
                        style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                  ])),
                  if (_fetching) const SizedBox(width: 20, height: 20,
              ...
              Expanded(
                ...
                // ── § 1 Deal Information ──────────────────────────
                const _SecHeader(title: 'Deal Information', icon: Icons.info_outline),
                ...
                const SizedBox(height: 6),
                Row(children: [
                  _CompactDrop(value: _currency, items: DealConstants.currencies, width: 110,
                      onChanged: (v) => setState(() => _currency = v)),
                  const SizedBox(width: 10),
                  Expanded(child: _tf(_value, '0.00', type: TextInputType.number)),
                ]),
                const SizedBox(height: 16),
                ...
                const SizedBox(height: 6),
                Row(children: [
                  _CompactDrop(value: _countryCode, items: DealConstants.countryCodes, width: 110,
                      onChanged: (v) => setState(() => _countryCode = v)),
                  const SizedBox(width: 10),
                  Expanded(child: _tf(_phone, '9876543210', type: TextInputType.phone)),
                ]),
                const SizedBox(height: 16),

                const _FieldLabel('Email', required: true),
                const SizedBox(height: 6),
                _tf(_email, 'deal@company.com', type: TextInputType.emailAddress),
                const SizedBox(height: 16),

                const _FieldLabel('Stage'),
                const SizedBox(height: 6),
                _FullDrop(value: _stage, items: DealConstants.stages, icon: Icons.flag_outlined,
                ...
                const _SecHeader(title: 'Business Details', icon: Icons.business_outlined),
                const SizedBox(height: 14),
                const _FieldLabel('Industry'),
                const SizedBox(height: 6),
                _FullDrop(value: _industry, items: DealConstants.industries,
                ...
                const SizedBox(height: 6),
                _FullDrop(value: _source, items: AppConstants.leadSources,
                ...
                const SizedBox(height: 6),
                _FullDrop(value: _country, items: AppConstants.countries,
                    icon: Icons.public_outlined,
                    onChanged: (v) => setState(() => _country = v)),
                const SizedBox(height: 24),

                // ── § 2 Follow-up ─────────────────────────────────
                const _SecHeader(title: 'Follow-up', icon: Icons.alarm_outlined),
                const SizedBox(height: 14),
                ...
                Row(children: [
                  ...
                  Expanded(child: GestureDetector(onTap: _pickTime,
                    child: _pickerBox(icon: Icons.access_time_outlined,
                      label: _followUpTime != null ? _followUpTime!.format(context) : 'Select time',
                ...
                if (_followUpDate == null && _followUpTime == null)
                  Padding(padding: const EdgeInsets.only(top: 8),
                    child: Row(children: [
                      ...
                      const SizedBox(width: 6),
                      Expanded(child: Text('Optional: Set a reminder for follow-up',
                  ...
                  Padding(padding: const EdgeInsets.only(top: 8),
                    child: GestureDetector(
                      onTap: () => setState(() { _followUpDate = null; _followUpTime = null; }),
                ...
                const SizedBox(height: 6),
                _tf(_followUpComment, 'Add a note about this follow-up…', maxLines: 3),
                const SizedBox(height: 24),

                // ── § 3 Management ────────────────────────────────
                const _SecHeader(title: 'Management', icon: Icons.tune_outlined),
                const SizedBox(height: 14),
                ...
                widget.users.isEmpty
                    ...
                    : _FullDrop(value: _assignTo ?? widget.users.first,
                        items: widget.users, icon: Icons.person_outline,
                ...
                const SizedBox(height: 6),
                _tf(_notes, 'Any additional context…', maxLines: 4),
                const SizedBox(height: 24),

                // ── § 4 Attachments ───────────────────────────────
                const _SecHeader(title: 'Attachments', icon: Icons.attach_file_rounded),
                const SizedBox(height: 14),

                GestureDetector(
                  onTap: _attachmentSlotsUsed < 5 ? _pickFiles : null,
                  child: Container(
                    width: double.infinity,
                    padding: const EdgeInsets.symmetric(vertical: 18),
                    decoration: BoxDecoration(
                      color: AppColors.surface, borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: _attachmentSlotsUsed < 5
                          ? AppColors.primary.withOpacity(0.4) : AppColors.border)),
                    child: Column(children: [
                      Icon(Icons.cloud_upload_outlined, size: 32,
                          color: _attachmentSlotsUsed < 5 ? AppColors.primary : AppColors.textHint),
                      const SizedBox(height: 8),
                      Text(_attachmentSlotsUsed < 5 ? 'Tap to upload files' : 'Maximum 5 files reached',
                          style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                              color: _attachmentSlotsUsed < 5 ? AppColors.primary : AppColors.textHint)),
                      const SizedBox(height: 4),
                      const Text('Any file type · Max 5 MB per file · Up to 5 files',
                ...
                if (_attachments.isNotEmpty) ...[
                  ...
                  child: Row(children: [
                    Container(padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(color: AppColors.primaryLight,
                          borderRadius: BorderRadius.circular(8)),
                      child: Icon(_fileIcon(e.value.name), size: 16, color: AppColors.primary)),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    GestureDetector(onTap: () => _removeAttachment(e.key),
                ...
                Row(children: [
                  ...
                  Expanded(child: ElevatedButton.icon(
                    onPressed: (_saving || _fetching) ? null : _save,
                    style: _isEdit ? ElevatedButton.styleFrom(
                        backgroundColor: AppColors.warning) : null,
                    icon: _saving
                        ? const SizedBox(width: 16, height: 16,
                            child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                        : Icon(_isEdit ? Icons.check_outlined : Icons.save_outlined, size: 16),
                    label: Text(_saving
                        ? (_isEdit ? 'Updating…' : 'Saving…')
                        : (_isEdit ? 'Update Deal' : 'Save Deal')))),
                ]),
                const SizedBox(height: 24),
              ])),
            ])))),
  ...
  Widget _pickerBox({required IconData icon, required String label, required bool active}) =>
...
class _SearchBar extends StatelessWidget {
  ...
  final void Function(String) onChanged; final VoidCallback onClear;
  const _SearchBar({required this.controller, required this.hint,
      required this.onChanged, required this.onClear});
  @override
  Widget build(BuildContext context) => SizedBox(height: 42,
...
class _FDrop extends StatelessWidget {
  final String value; final List<String> items; final IconData icon;
  final void Function(String) onChanged; final String Function(String)? labelBuilder;
  const _FDrop({required this.value, required this.items, required this.icon,
      required this.onChanged, this.labelBuilder});
  ...
  Widget build(BuildContext context) => Container(height: 40,
    ...
    decoration: BoxDecoration(
      ...
      borderRadius: BorderRadius.circular(10),
      border: Border.all(color: _active ? AppColors.primary : AppColors.border)),
    child: DropdownButtonHideUnderline(child: DropdownButton<String>(
      ...
      items: items.map((e) => DropdownMenuItem(value: e,
        child: Row(children: [
          ...
          Expanded(child: Text(labelBuilder != null ? labelBuilder!(e) : e,
...
class _FieldLabel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Text.rich(TextSpan(text: text,
    style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
        color: AppColors.textSecondary),
    children: required ? [const TextSpan(text: ' *',
        style: TextStyle(color: AppColors.danger))] : []));
}

class _CompactDrop extends StatelessWidget {
  final String value; final List<String> items;
  final double width; final void Function(String) onChanged;
  const _CompactDrop({required this.value, required this.items,
      required this.width, required this.onChanged});
  @override
  Widget build(BuildContext context) => Container(width: width, height: 48,
...
class _FullDrop extends StatelessWidget {
  final String value; final List<String> items; final IconData icon;
  final void Function(String) onChanged; final String Function(String)? labelBuilder;
  const _FullDrop({required this.value, required this.items, required this.icon,
      required this.onChanged, this.labelBuilder});
  @override
  Widget build(BuildContext context) => Container(height: 48,
    padding: const EdgeInsets.symmetric(horizontal: 14),
    decoration: BoxDecoration(color: AppColors.surface,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: AppColors.border)),
